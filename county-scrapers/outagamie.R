get_outagamie <- function(url) {
  ...
  map(pages, function(page_text) {
    ...
    # Ward name: first line starting with Town/Village/City
    ward_idx <- which(str_detect(lines, "^(Town|Village|City) of"))
    ...
    map(vf_indices, function(vf_idx) {
      ...
      if (race_name %in% c("Party Preference", "Statistics")) return(tibble())

...
source("county-scrapers/waukesha.R")

raw.results <- get_waukesha("https://electionresults.waukeshacounty.gov/contests.aspx?contest=-1") |>
...
saveRDS(raw.results, "processed/aug2026/waukesha.rds")
...
raw.results |>
  filter(office == "Representative to the Assembly District 13",
...
# 
# saveRDS(reporting.units.polygons, "county-rep-unit-polygons/waukesha.rds")
# ###############################################################################

################################################################################
rep.unit.polygons <- read_rds("county-rep-unit-polygons/waukesha.rds") |>
  mutate(
    circle_center = local({
      circ <- geometry |>
        st_transform(3070) |>
...
create_pmtiles <- function(df, office){
  tiles.dir <- paste0("tiles/waukesha/", office)
...
cnty.shp <- tigris::counties(cb = T) |> filter(GEOID == 55133) |> st_transform(crs = 4326)
...
pm.gov <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/waukesha/gov/votes.pmtiles", gov)

htmlwidgets::saveWidget(pm.gov, "pm-maps/waukesha/gov.html", selfcontained = TRUE)

...
#   # Fix period-as-comma or ampersand-as-comma typo (e.g. "4.9" → "4,9" or "6-7 & 17" → "6-7, 17")
#   ru <- str_replace_all(ru, "\\.|\\s*&\\s*", ",")
# 
...
# 
# saveRDS(reporting.units.polygons, "county-rep-unit-polygons/outagamie.rds")
# ###############################################################################

################################################################################
rep.unit.polygons <- read_rds("county-rep-unit-polygons/outagamie.rds") |>
  mutate(
    circle_center = local({
      circ <- geometry |>
        st_transform(3070) |>
...
create_pmtiles <- function(df, office){
  tiles.dir <- paste0("tiles/outagamie/", office)
...
cnty.shp <- tigris::counties(cb = T) |> filter(GEOID == 55087) |> st_transform(crs = 4326)
...
pm.gov <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/outagamie/gov/votes.pmtiles", gov)

htmlwidgets::saveWidget(pm.gov, "pm-maps/outagamie/gov.html", selfcontained = TRUE)

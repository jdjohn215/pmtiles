...
raw.results |>
  filter(office == "Representative in Congress, District 4",
...
raw.results |>
  filter(office == "Representative to the Assembly, District 21",
         party %in% c("Democratic", "Republican")) |>
  group_by(party, candidate) |>
  summarise(votes = sum(votes)) |>
  mutate(pct = votes/sum(votes)*100)
...
raw.results |>
  filter(office == "Representative to the Assembly, District 61",
         party %in% c("Democratic", "Republican")) |>
  group_by(party, candidate) |>
  summarise(votes = sum(votes)) |>
  mutate(pct = votes/sum(votes)*100)
...
raw.results |>
  filter(office == "Representative to the Assembly, District 19",
         party %in% c("Democratic", "Republican")) |>
  group_by(party, candidate) |>
  summarise(votes = sum(votes)) |>
  mutate(pct = votes/sum(votes)*100)
...
raw.results |>
  filter(office == "Representative to the Assembly, District 8",
         party %in% c("Democratic", "Republican")) |>
  group_by(party, candidate) |>
  summarise(votes = sum(votes)) |>
  mutate(pct = votes/sum(votes)*100)
...
#   separate(ward, into = c("MCD_NAME", "reporting_unit"), sep = "\\bWard\\b|\\bWards\\b") |>
#   mutate(CTV = str_sub(MCD_NAME, 1, 1),
#          across(where(is.character), str_squish),
#          across(where(is.character), str_to_upper),
#          MCD_NAME = str_remove(MCD_NAME, "VILLAGE OF |CITY OF |TOWN OF |V. |C. "),
#          MCD_NAME = str_replace(MCD_NAME, "SO. MILW.", "SOUTH MILWAUKEE")) |>
...
# 
# saveRDS(reporting.units.polygons, "county-rep-unit-polygons/milwaukee.rds")
################################################################################

################################################################################
rep.unit.polygons <- read_rds("county-rep-unit-polygons/milwaukee.rds") |>
  mutate(
    circle_center = local({
      circ <- geometry |>
        st_transform(3070) |>
...
create_pmtiles <- function(df, office){
  tiles.dir <- paste0("tiles/milwaukee/", office)
...
cnty.shp <- tigris::counties(cb = T) |> filter(GEOID == 55079) |> st_transform(crs = 4326)
...
pm.gov <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/gov/votes.pmtiles", gov)
pm.ad8 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/ad8/votes.pmtiles", ad8)
pm.ad19 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/ad19/votes.pmtiles", ad19)
pm.ad21 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/ad21/votes.pmtiles", ad21)
pm.ad61 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/ad61/votes.pmtiles", ad61)
pm.cd4 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/milwaukee/cd4/votes.pmtiles", cd4)

htmlwidgets::saveWidget(pm.gov, "pm-maps/milwaukee/gov.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.ad8, "pm-maps/milwaukee/ad8.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.ad19, "pm-maps/milwaukee/ad19.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.ad21, "pm-maps/milwaukee/ad21.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.ad61, "pm-maps/milwaukee/ad61.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.cd4, "pm-maps/milwaukee/cd4.html", selfcontained = TRUE)

...
raw.results |>
  filter(office == "Representative to the Assembly District 76",
...
#   separate(ward, into = c("MCD_NAME", "reporting_unit"), sep = "\\bWard\\b|\\bWards\\b|\\bWd\\b|\\bWds\\b") |>
#   mutate(CTV = str_sub(MCD_NAME, 1, 1),
#          across(where(is.character), str_squish),
#          across(where(is.character), str_to_upper),
#          MCD_NAME = word(MCD_NAME, 2, -1)) |>
...
# 
# saveRDS(reporting.units.polygons, "county-rep-unit-polygons/dane.rds")
# ###############################################################################

################################################################################
rep.unit.polygons <- read_rds("county-rep-unit-polygons/dane.rds") |>
  mutate(
    circle_center = local({
      circ <- geometry |>
        st_transform(3070) |>
...
create_pmtiles <- function(df, office){
  tiles.dir <- paste0("tiles/dane/", office)
...
cnty.shp <- tigris::counties(cb = T) |> filter(GEOID == 55025) |> st_transform(crs = 4326)
...
pm.gov <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/dane/gov/votes.pmtiles", gov)
pm.ad76 <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/dane/ad76/votes.pmtiles", ad76)

htmlwidgets::saveWidget(pm.gov, "pm-maps/dane/gov.html", selfcontained = TRUE)
htmlwidgets::saveWidget(pm.ad76, "pm-maps/dane/ad76.html", selfcontained = TRUE)

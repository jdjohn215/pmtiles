...
#          MCD_NAME = str_replace(MCD_NAME, "MT. PLEASANT", "MOUNT PLEASANT")) |>
...
# 
# saveRDS(reporting.units.polygons, "county-rep-unit-polygons/racine.rds")
# ###############################################################################

################################################################################
rep.unit.polygons <- read_rds("county-rep-unit-polygons/racine.rds") |>
  mutate(
    circle_center = local({
      circ <- geometry |>
        st_transform(3070) |>
...
create_pmtiles <- function(df, office){
  tiles.dir <- paste0("tiles/racine/", office)
...
cnty.shp <- tigris::counties(cb = T) |> filter(GEOID == 55101) |> st_transform(crs = 4326)
...
pm.gov <- pmtiles_map("https://jdjohn215.github.io/pmtiles/tiles/racine/gov/votes.pmtiles", gov)

htmlwidgets::saveWidget(pm.gov, "pm-maps/racine/gov.html", selfcontained = TRUE)
